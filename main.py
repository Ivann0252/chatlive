    ...
    try:
        ...
        for r in reversed(res.data or []):
            ...
            result.append({
                ...
                "text": r["text"], "type": r.get("msg_type","message"),
                "edited": r.get("edited",False), "deleted": False, "pinned": r.get("pinned",False),
                "time": r.get("created_at","")[-8:-3], "avatar": r.get("avatar"),
                "reply_to": reply, "file_url": r.get("file_url"), "file_type": r.get("file_type")
            })
        return result
    except Exception as e: print(f"history error: {e}"); return []
...
def db_save_message(room, username, color, text, avatar=None, msg_type="message", reply_to=None, file_url=None, file_type=None):
    try:
        ...
        try:
            ...
            if u.data: supabase.table("users").update({"msg_count": (u.data[0].get("msg_count",0) or 0)+1}).eq("username", username).execute()
        except: pass
        return res.data[0]["id"] if res.data else None
    except Exception as e: print(f"save_msg error: {e}"); return None
...
def db_purge_ephemeral():
    try:
        ...
        for u in users:
            ...
            days = u.get("ephemeral_days", 7) or 7
            cutoff = (datetime.now() - timedelta(days=days)).strftime("%Y-%m-%d %H:%M:%S")
            supabase.table("messages").update({"deleted": True}).eq("username", u["username"]).lt("created_at", cutoff).eq("msg_type","message").execute()
...
def db_get_dm_history(conv_id, limit=60):
    try:
        ...
        result = []
        for r in reversed(res.data or []):
            result.append({"id":r["id"],"from_user":r["from_user"],"to_user":r["to_user"],
                "text":"[supprimé]" if r.get("deleted") else r.get("text",""),
                "deleted":r.get("deleted",False),"avatar":r.get("avatar"),"color":r.get("color"),
                "time":r.get("created_at","")[-8:-3]})
        return result
    except: return []

def db_save_dm(conv_id, from_user, to_user, text, avatar, color):
    try:
        res = supabase.table("dm_messages").insert({"conversation_id":conv_id,"from_user":from_user,"to_user":to_user,"text":text,"avatar":avatar,"color":color,"deleted":False,"created_at":now_full()}).execute()
        return res.data[0]["id"] if res.data else None
    except: return None
    ...
    def disconnect(self, ws, room):
        ...
        if room in self.rooms:
            for c in self.rooms[room]:
                if c["ws"] == ws: uname = c["username"]; break
            self.rooms[room] = [c for c in self.rooms[room] if c["ws"] != ws]
        if uname in self.connected: del self.connected[uname]
        return uname
...
async def auth_callback(code: str = None, error: str = None):
    ...
    from urllib.parse import urlencode
    p = urlencode({"username":user.get("username",username),"color":user.get("color","#1D9E75"),"accent":user.get("accent_color","#1D9E75"),"avatar":avatar,"google_id":google_id})
...
async def ws_main(ws: WebSocket, room: str, username: str, color: str):
    ...
    # Welcome message (only for user)
    room_data = supabase.table("rooms").select("label,welcome_msg,is_private").eq("name",room).execute().data
    room_label = room_data[0].get("label",room) if room_data else room
    welcome_msg = room_data[0].get("welcome_msg") if room_data else None
    ...
    try:
        while True:
            ...
            if mtype == "message":
                ...
                payload = {"type":"message","id":msg_id,"username":uname,"color":ucolor,"avatar":uavatar,"text":text,"time":now_time(),"reply_to":reply_to,"edited":False,"file_url":file_url,"file_type":file_type,"members":mgr.get_members(room),"online":mgr.count(room)}
                ...
                if "@" in text:
                    ...
                    for m in mentions:
                        if m != uname:
                            await mgr.send_to(m, {"type":"mention","from":uname,"room":room,"room_label":room_label,"text":text})
                ...
                try:
                    ...
                    if (cnt.count or 0) >= 1: await ws.send_text(json.dumps({"type":"error","text":"Tu as déjà un salon privé."})); continue
                ...
                if db_get_role(room,uname) not in ["admin","owner"]: await ws.send_text(json.dumps({"type":"error","text":"Réservé aux admins."})); continue
                target = msg.get("username","").strip(); muted = bool(msg.get("muted",True))
                try:
                ...
                try:
                    ...
                    await ws.send_text(json.dumps({"type":"info","text":"Chat remis à zéro !"}))
                except: pass
                ...
                try:
                    ...
                    await ws.send_text(json.dumps({"type":"room_stats","stats":{"messages":mc.count or 0,"members":(mem.count or 0)+1}}))
                except: pass

            elif mtype == "update_status":
                try: supabase.table("users").update({"status":msg.get("status","online")}).eq("username",uname).execute()
                except: pass
                ...
                updates = {}
                if "bio" in msg: updates["bio"] = msg["bio"][:120]
                ...
                if updates:
                    ...
                    await ws.send_text(json.dumps({"type":"info","text":"Profil mis à jour !"}))
                    if "color" in updates or "accent_color" in updates:
                        await ws.send_text(json.dumps({"type":"profile_updated","updates":updates}))

            elif mtype == "update_settings":
                updates = {}
                for k in ["keep_messages","ephemeral_days","notif_mentions","notif_messages","notif_dm","notif_requests","notif_join","notif_sound"]:
                    if k in msg: updates[k] = msg[k]
                if updates:
                    try: supabase.table("users").update(updates).eq("username",uname).execute()
                    except: pass
                ...
                try:
                    ...
                    if res.data:
                        ...
                        await ws.send_text(json.dumps({"type":"profile","user":{"username":u2["username"],"color":u2.get("color","#1D9E75"),"accent_color":u2.get("accent_color","#1D9E75"),"avatar":u2.get("avatar"),"status":u2.get("status","online"),"bio":u2.get("bio",""),"joined_at":ja_str,"last_active":la_str,"email":u2.get("email",""),"msg_count":u2.get("msg_count",0)}}))
                except: pass
    ...
    try:
        while True:
            ...
            if mtype == "dm_request":
                ...
                if not target or target == username: await ws.send_text(json.dumps({"type":"error","text":"Pseudo invalide."})); continue
                try:
                    u = supabase.table("users").select("username").eq("username",target).execute()
                    if not u.data: await ws.send_text(json.dumps({"type":"error","text":f"'{target}' n'existe pas."})); continue
                except: pass
                try:
                    existing = supabase.table("dm_requests").select("*").or_(f"and(from_user.eq.{username},to_user.eq.{target}),and(from_user.eq.{target},to_user.eq.{username})").execute()
                    if existing.data:
                ...
                except Exception as e: await ws.send_text(json.dumps({"type":"error","text":"Erreur."}))
                ...
                target = msg.get("to","").strip(); text = msg.get("text","").strip()
                if not text or not target: continue
                try:
                    req = supabase.table("dm_requests").select("status").or_(f"and(from_user.eq.{username},to_user.eq.{target}),and(from_user.eq.{target},to_user.eq.{username})").execute()
                ...
                msg_id = msg.get("msg_id"); target = msg.get("to","").strip()
                try:
